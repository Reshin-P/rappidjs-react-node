...
function App() {
  ...
  useEffect(() => {
    ...
    window.ononline = async (event) => {
      ...
      try {
        const { data } = await axios.post(`/paper/${params.id}`,{ data: jsonString });
      ...
      <Row className="main-row ">
        ...
        <Col id="total" className="col2" sm={12} md={8} lg={8} xl={8}>
          <div className="tools">
            ...
            <Button style={{marginRight:"400px"}} onClick={zoomIn} variant="outline-warning">
              <img width={"30px"} height={"30px"} src="/images/in.png"></img>
            </Button>
          <Link to={`/diagram/${params.id}`} >  <Button style={{marginRight:"400px"}} onClick={zoomIn} variant="outline-warning">
             Create Diagram
            </Button> </Link>
